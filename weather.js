...
function getWeather(lat,lon, imgElement, textElement){
    ...
    let apiURL = "https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtNcst?serviceKey=wJB5u4Lk3FG95bFUSRipiX938gVIdiLfukQ806B8ql2H2ud9dnffH7kvGhC2yrVevlN0ewKzyeISd3DWI5QVVg%3D%3D&pageNo=1&numOfRows=1000&dataType=JSON&base_date=" +formattedDate + "&base_time=" + formattedTime +"&nx=" + nx + "&ny=" + ny;
    ...
    $.getJSON(apiURL, function (data) {
        ...
        for (var i = 0; i < dataObject.length; i++) {
            ...
            if(item.category == "T1H"){content = item.obsrValue;}
            if(item.category == "PTY"){weather = item.obsrValue;}
        }
